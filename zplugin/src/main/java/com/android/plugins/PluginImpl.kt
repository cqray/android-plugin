...
//            println("==============" + Gson().toJson(temp))
    ...
    private fun loadCoroutine(options: PluginOptions?, configuration: GradleBuildConfiguration) {
        ...
        if (isSdkEnabled(options, configuration) { it?.coroutineEnabled }) {
            project.dependencies.add("api", Sdks.androidx.coroutinesAndroid)
            project.dependencies.add("api", Sdks.androidx.lifecycleLivedataKtx)
            project.dependencies.add("api", Sdks.androidx.lifecycleRuntimeKtx)
            project.dependencies.add("api", Sdks.androidx.lifecycleViewModelKtx)
        }
    }
    ...
    private fun loadRetrofit(options: PluginOptions?, configuration: GradleBuildConfiguration) {
        ...
        if (isSdkEnabled(options, configuration) { it?.retrofitEnabled }) {
            ...
            project.dependencies.add("api", Sdks.http.okHttp3Log)
            if (rxjava2Enabled) project.dependencies.add("api", Sdks.http.retrofitRx2Adapter)
            if (rxjava3Enabled) project.dependencies.add("api", Sdks.http.retrofitRx3Adapter)
        }
    }
    ...
    private fun loadRoom(options: PluginOptions?, configuration: GradleBuildConfiguration) {
        ...
        if (isSdkEnabled(options, configuration) { it?.roomEnabled }) {
            ...
            project.dependencies.add("annotationProcessor", Sdks.androidx.roomCompiler)
            runCatching { project.dependencies.add("kapt", Sdks.androidx.roomCompiler) }
            // 启用rxjava2
            if (rxjava2Enabled) project.dependencies.add("api", Sdks.androidx.roomRxjava2)
            // 启用rxjava3
            if (rxjava3Enabled) project.dependencies.add("api", Sdks.androidx.roomRxjava3)
            // 启用协程
            if (coroutineEnabled) project.dependencies.add("api", Sdks.androidx.roomCoroutine)
        }
    }
